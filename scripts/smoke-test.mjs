...
const share = buildShareText(cases.map((caseFile) => evaluateOrder(caseFile, orderedArtifacts(caseFile).map((item) => item.id))));
assert.match(share, /Time Heist Shuffle 2026-05-23/);
assert.match(share, /3\/3 cases solved/);

console.log("Smoke tests passed: cases, scoring, moves, swaps, and share text.");
